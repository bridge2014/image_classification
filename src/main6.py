...
print(f"class name list : {class_names}")
...
train_generator = train_datagen.flow_from_directory(
    ...
    seed=42,      # good for reproducibility
    shuffle=True  # usually keep True for training
)
...
x = Dropout(0.3)(x)             # helps prevent overfitting (optional)
...
# Optional: Fine-tune (unfreeze last 30 layers for better adaptation)
...
model.fit(train_generator, 
          validation_data=val_generator,
          epochs=previous_epochs + 20,
          initial_epoch=previous_epochs)  # Additional epochs
...
model.save('resnet50_cancer_classifier6.h5')
...
plt.savefig('accuracy_plot6.png', dpi=150, bbox_inches='tight')
#plt.show()