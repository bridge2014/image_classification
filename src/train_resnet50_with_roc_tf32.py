...
def build_model(img_size, num_classes):
    ...
    x = base(x, training=True)  # Allow BN to update
    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dropout(0.3)(x)
    x = layers.Dense(512, activation="relu")(x)  # More capacity
    x = layers.Dropout(0.3)(x)
...
def get_class_weights(train_dir: str) -> Dict[int, float]:
    ...
    if not class_count:
        print("Warning: Could not calculate class weights automatically")
        return None
    ...
    for idx, (class_name, count) in enumerate(sorted(class_count.items())):
        weight = total_samples / (num_classes * count) if count > 0 else 1.0
        class_weights[idx] = weight
    
    # Normalize weights
    total_weight = sum(class_weights.values())
    class_weights = {k: v / total_weight * num_classes for k, v in class_weights.items()}
...
def main():
    ...
    '''
    # One way to calculate class_weight
    ...
    # Another way to calculate class_weight 
